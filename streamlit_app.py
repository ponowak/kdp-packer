...
st.set_page_config(page_title="7-Backend Packer", page_icon="📚", layout="centered")
...
# Pobieranie aktualnych kredytów
try:
...
with col2:
    st.write("") 
    lang_choice = st.radio("Język:", ["🇺🇸 EN", "🇩🇪 DE", "🇫🇷 FR", "🇪🇸 ES", "🇮🇹 IT"], horizontal=True, label_visibility="collapsed")
...
def pack_keywords(title, raw_keywords_text):
    ...
    if current_line and len(lines) < 7: lines.append(" ".join(current_line))
    while len(lines) < 7: lines.append("")
    return lines

if st.button(f"Generuj i Pakuj ({target_language})", type="primary"):
    if current_credits <= 0:
        st.error("🚫 Wykorzystałeś wszystkie tokeny. Potrzebujesz więcej, by kontynuować pracę.")
        
        # Link do kasy LemonSqueezy (z przekazaniem emaila i ID użytkownika, by webhook wiedział kto płaci)
        safe_email = urllib.parse.quote(st.session_state.user.email)
        user_id = st.session_state.user.id
        checkout_url = f"https://7backend.lemonsqueezy.com/checkout/buy/c849be9a-661b-4b1c-844f-10e88367d7d3?checkout[email]={safe_email}&checkout[custom][user_id]={user_id}"
        
        st.markdown(f"""
        <a href="{checkout_url}" target="_blank">
            <button style="background-color:#4CAF50; color:white; padding:15px 32px; font-size:18px; border:none; border-radius:8px; cursor:pointer; width:100%;">
                🛒 Kup pakiet 10 Tokenów
            </button>
        </a>
        ...
        with st.spinner(f'AI generuje słowa w języku: {target_language}...'):
            try:
                ...
                for i, line in enumerate(lines):
                    ...
                    with col1: st.code(line, language="text")
                    with col2: st.caption(f"{len(line)}/50 znaków")
                    export_text += f"Pole {i+1}: {line}\n"
                
                st.download_button("📥 Pobierz wyniki (.TXT)", data=export_text.encode('utf-8'), file_name="kdp_keywords.txt", mime="text/plain")
                with st.expander("Zobacz surowe słowa (AI)"): st.write(raw_keywords)
